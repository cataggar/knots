...
pub inline fn all(v: f32) BorderWidth {
    const w = @max(0, v);
    return .{ .value = .{ w, w, w, w } };
}

pub inline fn edges(top: f32, right: f32, bottom: f32, left: f32) BorderWidth {
    return .{ .value = .{
        @max(0, top),
        @max(0, right),
        @max(0, bottom),
