...
// Same JS `KeyboardEvent.code` string values that Emscripten's
// `EmscriptenKeyboardEvent.code` field already carries (Emscripten copies the
// DOM string verbatim), so this table is identical to
// `emscripten/keymap.zig`'s.
...
pub fn translateCode(code: []const u8) ?Key {
